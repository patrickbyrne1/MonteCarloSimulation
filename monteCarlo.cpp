...
// how many tries to discover the maximum performance (ie number of laser shots):
#ifndef NUMTRIES
...
int main( int argc, char *argv[ ] )
{
	...
	for( int t = 0; t < NUMTRIES; t++ )
	{
		...
		for( int n = 0; n < NUMTRIALS; n++ )
		{
			...
			// if d is less than zero, the laser missed
			if(d < 0) {
				continue;
			}
			...
			// if tmin is less than zero, the circle engulfed the laser
			if(tmin < 0) {
				continue;
			}
			...
			// if tt is greater than or equal to zero, the beam was reflected down and hits the plate
			if(tt >= 0) {
				numHits++;
			}
		...
		currentProb = (float)numHits/(float)NUMTRIALS; // This will be the same for all trials b/c the number of hits is the same
		
		
	}
	//printf("Number of hits: %d\n",numHits);
	printf("%d,%d,%4.2lf,%4.2lf\n", NUMT, NUMTRIALS, currentProb, maxPerformance);
}
